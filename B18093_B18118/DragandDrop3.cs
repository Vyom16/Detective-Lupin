...
public class DragandDrop3 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                Debug.Log("Vyom");
                if (!hit.transform.GetComponent<PieceScript3>().InRightPosition)
                {
                    Debug.Log("Vyom2");
                    SelectedPiece = hit.transform.gameObject;
                    SelectedPiece.GetComponent<PieceScript3>().Selected = true;
                }
            }
        }
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                SelectedPiece.GetComponent<PieceScript3>().Selected = false;
                SelectedPiece = null;
            }
        }
