...
public class DragandDrop2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                Debug.Log("Vyom");
                if (!hit.transform.GetComponent<PieceScript2>().InRightPosition)
                {
                    Debug.Log("Vyom2");
                    SelectedPiece = hit.transform.gameObject;
                    SelectedPiece.GetComponent<PieceScript2>().Selected = true;
                }
            }
        }
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                SelectedPiece.GetComponent<PieceScript2>().Selected = false;
                SelectedPiece = null;
            }
        }
